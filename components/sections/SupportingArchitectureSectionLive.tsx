...
const SLIDES = [
  ...
  { number: '02', title: 'Biosaccharide', benefit: 'MOISTURE RETENTION', description: 'A plant-derived polysaccharide that forms a lightweight moisture film to reduce water loss and support skin comfort following aesthetic treatment.', details: ['Moisture retention', 'Reduced water loss', 'Skin comfort'], illustration: 'biosaccharide' },
  ...
  return (
    <section id="sa-section" ref={sectionRef} style={{ height: '800vh', backgroundColor: '#F7E8EC' }}>
      <div ref={stageRef} style={{ position: 'sticky', top: 0, height: '100vh', backgroundColor: AMBIENT_BG[0], display: 'flex', flexDirection: 'column', paddingTop: 88, overflow: 'hidden' }}>
        <div style={{ flexShrink: 0, textAlign: 'center', paddingBottom: 14, zIndex: 10 }}><h2 className="font-serif" style={{ fontSize: 'clamp(22px, 2.6vw, 36px)', fontWeight: 400, color: '#732C3F', lineHeight: 1.1, margin: '0 0 10px' }}>Supporting Architecture</h2><p style={{ fontSize: 11, letterSpacing: '0.18em', textTransform: 'uppercase', color: '#C57C8A', margin: 0, fontFamily: SANS }}>Formula Components</p></div>
        <div style={{ position: 'relative', flex: 1, overflow: 'hidden' }}>
          {SLIDES.map((slide, i) => <div key={i} ref={(el) => { slideRefs.current[i] = el }} style={{ position: 'absolute', inset: 0, opacity: i === 0 ? 1 : 0, pointerEvents: i === 0 ? 'auto' : 'none', zIndex: i === 0 ? 5 : 0 }}>
            <div style={{ position: 'absolute', left: '50%', top: slide.illustration === 'adenosine' || slide.illustration === 'allantoin' ? '76%' : '68%', transform: 'translateX(-50%)', zIndex: 4, pointerEvents: 'auto', textAlign: 'center', whiteSpace: 'nowrap' }}><h3 className="font-serif" style={{ fontSize: 'clamp(48px, 7vw, 110px)', fontWeight: 400, color: '#732C3F', opacity: 0.18, letterSpacing: 0, lineHeight: 1, margin: 0, whiteSpace: 'nowrap' }}>{slide.title}</h3></div>
            ...
            <div style={{ position: 'absolute', bottom: 'max(54px, 7vh)', left: '6.2vw', zIndex: 4, width: 'min(320px, 27vw)' }}>{slide.details.map((detail, d) => <div key={detail} style={{ display: 'grid', gridTemplateColumns: '28px 26px 1fr', alignItems: 'center', columnGap: 12, paddingBottom: d < slide.details.length - 1 ? 11 : 0, marginBottom: d < slide.details.length - 1 ? 11 : 0, borderBottom: d < slide.details.length - 1 ? '1px solid rgba(115,44,63,0.1)' : undefined }}><DetailIcon detail={detail} index={d} /><div style={{ width: 26, height: 1, backgroundColor: '#EDC967' }} /><span style={{ fontSize: 10, color: '#732C3F', opacity: 0.9, letterSpacing: '0.1em', lineHeight: 1.35, fontWeight: 700, textTransform: 'uppercase', fontFamily: SANS }}>{detail}</span></div>)}</div>
          </div>)}
        </div>
