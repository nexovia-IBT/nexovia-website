      ...
      <div ref={stageRef} style={{ position: 'sticky', top: 0, height: '100vh', backgroundColor: AMBIENT_BG[0], display: 'flex', flexDirection: 'column', paddingTop: 88, overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative', flex: 1, overflow: 'hidden' }}>
          {SLIDES.map((slide, i) => <div key={i} ref={(el) => { slideRefs.current[i] = el }} style={{ position: 'absolute', inset: 0, opacity: i === 0 ? 1 : 0, pointerEvents: i === 0 ? 'auto' : 'none', zIndex: i === 0 ? 5 : 0 }}>
            <div style={{ position: 'absolute', left: '52%', top: slide.illustration === 'adenosine' || slide.illustration === 'allantoin' ? '76%' : '68%', transform: 'translateX(-50%)', zIndex: 4, pointerEvents: 'auto', textAlign: 'center', whiteSpace: 'nowrap' }}><h3 className="font-serif" style={{ fontSize: 'clamp(48px, 7vw, 110px)', fontWeight: 400, color: '#732C3F', opacity: 0.18, letterSpacing: '-0.02em', lineHeight: 1, margin: 0, whiteSpace: 'nowrap' }}>{slide.title}</h3></div>
