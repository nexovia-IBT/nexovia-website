      ...
      <div ref={stageRef} style={{ position: 'sticky', top: 0, height: '100vh', backgroundColor: AMBIENT_BG[0], display: 'flex', flexDirection: 'column', paddingTop: 88, overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative', flex: 1, overflow: 'hidden' }}>
          {SLIDES.map((slide, i) => <div key={i} ref={(el) => { slideRefs.current[i] = el }} style={{ position: 'absolute', inset: 0, opacity: i === 0 ? 1 : 0, pointerEvents: i === 0 ? 'auto' : 'none', zIndex: i === 0 ? 5 : 0 }}>
            <div style={{ position: 'absolute', left: '50%', top: slide.illustration === 'adenosine' || slide.illustration === 'allantoin' ? '76%' : '68%', transform: 'translateX(-50%)', zIndex: 4, pointerEvents: 'auto', textAlign: 'center', whiteSpace: 'nowrap' }}><h3 className="font-serif" style={{ fontSize: 'clamp(48px, 7vw, 110px)', fontWeight: 400, color: '#732C3F', opacity: 0.18, letterSpacing: '-0.02em', lineHeight: 1, margin: 0, whiteSpace: 'nowrap' }}>{slide.title}</h3></div>
            ...
            <div style={{ position: 'absolute', bottom: 'max(48px, 6.5vh)', left: '5.8vw', zIndex: 4, width: 'min(350px, 29vw)' }}>{slide.details.map((detail, d) => <div key={detail} style={{ display: 'grid', gridTemplateColumns: '30px 28px minmax(0, 1fr)', alignItems: 'center', columnGap: 12, paddingBottom: d < slide.details.length - 1 ? 10 : 0, marginBottom: d < slide.details.length - 1 ? 10 : 0, borderBottom: d < slide.details.length - 1 ? '1px solid rgba(115,44,63,0.1)' : undefined }}><DetailIcon detail={detail} index={d} /><div style={{ width: 28, height: 1, backgroundColor: '#EDC967' }} /><span style={{ fontSize: 10.5, color: '#732C3F', opacity: 0.9, letterSpacing: '0.1em', lineHeight: 1.25, fontWeight: 700, textTransform: 'uppercase', fontFamily: SANS }}>{detail}</span></div>)}</div>
          </div>)}
        </div>
