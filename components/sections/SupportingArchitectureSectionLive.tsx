      ...
      <div ref={stageRef} style={{ position: 'sticky', top: 0, height: '100vh', backgroundColor: AMBIENT_BG[0], display: 'flex', flexDirection: 'column', paddingTop: 88, overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative', flex: 1, overflow: 'hidden' }}>
          {SLIDES.map((slide, i) => <div key={i} ref={(el) => { slideRefs.current[i] = el }} style={{ position: 'absolute', inset: 0, opacity: i === 0 ? 1 : 0, pointerEvents: i === 0 ? 'auto' : 'none', zIndex: i === 0 ? 5 : 0 }}>
            ...
            <div style={{ position: 'absolute', top: slide.illustration === 'botanical' ? '16%' : '17%', right: slide.illustration === 'botanical' ? '7.6vw' : '9vw', width: slide.illustration === 'botanical' ? 'min(340px, 28vw)' : 'min(285px, 24vw)', zIndex: 4, textAlign: 'right' }}><p style={{ fontSize: 11, letterSpacing: '0.13em', textTransform: 'uppercase', color: '#C57C8A', margin: '0 0 10px', fontFamily: SANS }}>{slide.benefit}</p><p style={{ fontSize: 14, lineHeight: 1.75, color: '#5A1F2E', opacity: 0.82, margin: 0, fontFamily: SANS }}>{slide.description}</p></div>
            <div style={{ position: 'absolute', bottom: 'max(52px, 7vh)', left: '4.8vw', zIndex: 4, width: 'min(292px, 24vw)' }}>{slide.details.map((detail, d) => <div key={detail} style={{ display: 'grid', gridTemplateColumns: '26px 22px minmax(0, 1fr)', alignItems: 'center', columnGap: 10, paddingBottom: d < slide.details.length - 1 ? 8 : 0, marginBottom: d < slide.details.length - 1 ? 8 : 0, borderBottom: d < slide.details.length - 1 ? '1px solid rgba(115,44,63,0.1)' : undefined }}><DetailIcon detail={detail} index={d} /><div style={{ width: 22, height: 1, backgroundColor: '#EDC967' }} /><span style={{ fontSize: 9.4, color: '#732C3F', opacity: 0.9, letterSpacing: '0.1em', lineHeight: 1.22, fontWeight: 700, textTransform: 'uppercase', fontFamily: SANS }}>{detail}</span></div>)}</div>
          </div>)}
        </div>
